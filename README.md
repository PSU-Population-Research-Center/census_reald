...
Estimates can be combined arithmetically to derive totals or proportions across or within groups or geographies. Following US Census Bureau guidance, approximations of standard errors for combined sums/differences and proportions/percents can be done by: 
...
- Derivation of Relative Standard Error (RSE) thresholds for data reliability: the confidence interval for \\(\hat{\theta}\\) is given by: \\(\hat{\theta} \pm z_{\alpha/2} \cdot \mathrm{SE}\\), where \\(\hat{\theta}\\) represents the point estimate and \\(\mathrm{SE}\\) the standard error of the estimate, and \\(z_{\alpha/2}\\) is the critical value for the standard normal distribution corresponding to the desired confidence level. The lower bound of the confidence interval includes zero when: \\(\hat{\theta} - z_{\alpha/2} \cdot \mathrm{SE} = 0\\). Rearranging terms, we get \\(\frac{\mathrm{SE}}{\hat{\theta}} = \frac{1}{z_{\alpha/2}}\\). For 95\% confidence, \\(\alpha = 0.05\\) and \\(z_{\alpha/2} \approx 1.96\\). Thus, \\(\mathrm{RSE} = \frac{1}{1.96} \approx 0.51\\).
