* v20: updated codes to ISO 639-3; ren langfnl>langsos; rescale repwgts; change donorLang to use 10-yrs data preceding.
*		note! this code assumes that langoha is a subset of langsos -- langoha can't be _more_ detailed.
...
// data prep
// -- attach agec3, agec5, agec11, lep, langc5, langc32, langc42, langsos (langs for report)
...
// -- recalculate pums language speakers according to b16001 totals
...
// -- rake to agec11/sex 
// -- rake to langsos from SOS analysis (mult, wash only)
...
prog def langControls
	...
	** consider: include 5ACS10, 5ACS15 PUMS for small populations
		tempfile tmp
		local T="B16001"
		censusapi, url("https://api.census.gov/data/2015/acs/acs5?get=group(`T')&for=county:*&in=state:41&key=$ckey")
		save `tmp', replace
		censusapi, url("https://api.census.gov/data/2015/acs/acs5?get=group(`T')&for=county:011&in=state:53&key=$ckey")
		append using `tmp'
		rename b16001_*e e*
		#delimit ;
		gen lc39eng0=e002;
	...
	** not present in all years so must fail gracefully when a county is missing data.
	...
	** consider ignoring county level because have already the 5-year PUMS, which is more reliable
		...
		if `year'<=2020 {
			tempfile tmp
			censusapi, url("https://api.census.gov/data/2019/acs/acs1?get=group(`T')&for=county:051&in=state:41&key=$ckey")
			cap assert b16001_001e<.
			if !_rc save `tmp', replace
			censusapi, url("https://api.census.gov/data/2018/acs/acs1?get=group(`T')&for=county:067&in=state:41&key=$ckey")
			cap assert b16001_001e<.
			cap append using `tmp'
			cap assert b16001_001e<.
			if !_rc save `tmp', replace
		}
		if `year'>2020 {
			tempfile tmp
			censusapi, url("https://api.census.gov/data/`year'/acs/acs1?get=group(`T')&for=county:051&in=state:41&key=$ckey")
			cap assert b16001_001e<.
			if !_rc save `tmp', replace
			censusapi, url("https://api.census.gov/data/`year'/acs/acs1?get=group(`T')&for=county:067&in=state:41&key=$ckey")
			cap assert b16001_001e<.
			cap append using `tmp'
			cap assert b16001_001e<.
			if !_rc save `tmp', replace
		}
		censusapi, url("https://api.census.gov/data/`year'/acs/acs5?get=group(`T')&for=state:41&key=$ckey")
		append using `tmp'
		rename b16001_*e e*
		#delimit ;
		gen lc42eng0=e002; 
	...
	** include data from 2015 EXCEPT for Wash, Mult, Clack, Clark; others get weighted average of 2010-15 and last
		...
		save sos/parse_farsi_chinese.dta, replace
...
cap prog drop langxwalk
prog def langxwalk
	** title: langxwalk
	** purpose: crosswalk Census language codes from 2016 onward (lanp16) to groups used in Census reports, 
	**		and to OHA REALD codes (currently, ISO 639-3)
	** author: sharygin@pdx.edu
	** note: this was done manually! results saved in csv here.
	** input data are the ACS code lists and etc. in the notes folder.
	** the csv is manually updated to include any new codes used in 5-yr PUMS from 2016 to the present.
	** eventually, the goal would be to generate it based on ranges of values to get a complete list of codes,
	** or to scrape the dictionary txt as used in the race/eth assignments.
	** however, the current workflow uses donor obs for every language with a code, so caution creating too many without a refactor!
	import delim using sos/prc_langxwalk.csv, clear
	save sos/prc_langxwalk.dta, replace
end
*langxwalk
...
prog def topdown42
	...
	** v03: converted to a program
	...
	forvalues y=2016/`year' {
		if `y'!=2020 { // no acs 1-year for 2020.
			censusapi, url("https://api.census.gov/data/`y'/acs/acs1?get=group(B16001)&for=state:41&key=$ckey")
			...
			censusapi, url("https://api.census.gov/data/`y'/acs/acs1?get=group(B16001)&for=county:051&in=state:41&key=$ckey")
			cap destring b*, replace ignore("BNnull")
			qui for var b*: replace X=. if X<0
			...
			censusapi, url("https://api.census.gov/data/`y'/acs/acs1?get=group(B16001)&for=county:067&in=state:41&key=$ckey")
			cap destring b*, replace ignore("BNnull")
			qui for var b*: replace X=. if X<0
			cap confirm var geo_id
			if !_rc {
	...
	forvalues y=2016/`year' {
		if `y'!=2020 {
			...
			if inrange(`y',2022,2031) {
				...
				censusapi, url("https://api.census.gov/data/`y'/acs/acs1/pums?get=`ST',AGEP,SEX,LANX,ENG,LANP,PWGTP&for=public%20use%20microdata%20area:05101,05102,05103,05105,05114,05116&in=state:41&key=$ckey") // multco 2020 pumas
				assert st<. // ensure it worked
				for any "v8" "publicusemicroda" "state" "st": cap drop X
				destring lanp, replace ignore("N")
				gen int year=`y'
				...
				append using SOS/td_pums.dta
				save SOS/td_pums.dta, replace 
				censusapi, url("https://api.census.gov/data/`y'/acs/acs1/pums?get=`ST',AGEP,SEX,LANX,ENG,LANP,PWGTP&for=public%20use%20microdata%20area:06720,06721,06722,06723,06724&in=state:41&key=$ckey") // washco 2020 pumas
				assert st<. // ensure it worked
				for any "v8" "publicusemicroda" "state" "st": cap drop X
				destring lanp, replace ignore("N")
				gen int year=`y'
	...
	** each share of langsos that is a lang42
	...
	*gsort stcofips langsos lep -year
	*by stcofips langsos lep : gen listme=_n
	*keep if listme<=5 // keep last 5 years' data
	ren lanp lanp16
	merge m:1 lanp16 using SOS/prc_langxwalk.dta, assert(2 3) keep(1 3) nogen keepus(langsos langc42)
	collapse (sum) perwt=pwgtp, by(stcofips lep langsos langc42)
	...
	** now, reshape wide so can merge with the lang42 table.
	keep shr langsos langc42 lep stcofips
	reshape wide shr, i(stcofips lep langc42) j(langsos)
	...
	** load lang42 table
	...
	drop n
	reshape long n@, i(stcofips year lep langc42) j(langsos) 
	collapse (sum) n, by(stcofips year lep langsos)
	replace n=round(n)
	merge m:m langsos using SOS/prc_langxwalk.dta, keepus(langsoslbl) keep(1 3) nogen
	** export table
	gsort stcofips lep -n
	by stcofips lep: gen rank=_n
	local y=substr("`year'",3,2)
	duplicates drop stcofips lep langsos n, force
	subsave stcofips lep langsos n using SOS/sos_lang42_topdown_5acs`y'.dta, replace
	** incorporate into OHA analysis
	** merge controls and add rake step for counts by langsos for multnomah and washington counties
...
** 	pulls preceding 5-year PUMS and anonymizes 10 yrs' data (starting in 2016, so that langcodes + pumas match)
**  doesn't pull 1-year PUMS because very rare langauges may be suppressed.
cap prog drop donorLang
prog def donorLang
	*cap confirm file sos/donor_lanp_`1'.dta
	*if _rc {
		local start=max(2016,`1'-5)
		local end=`1'
		local i=1
		foreach y in `start' `end' {
			foreach g in "for=state:41" "for=state:53" {
				...
				local vars="SERIALNO,AGEP,SEX,LANX,ENG,LANP,PWGTP" // not keeping PUMA or PUMA10/PUMA20 for donor obs...
				censusapi, url("https://api.census.gov/data/`y'/acs/acs5/pums?get=`vars'&LANX=1&`g'&key=$ckey") // filter for only non-English
				gen int year=real(substr(serialno,1,4)) // year of svy out of a 5-yr svy
				drop if year==`start' & inrange(year,`end'-4,`end') // drop from years from earlier 5-year that would overlap with the latest 5-year period.
				cap drop v* 
				assert lanp<. // ensure it worked
				destring lanp, replace ignore("N")
				ren lanp lanp16
				merge m:1 lanp16 using SOS/prc_langxwalk.dta, assert(2 3) keep(3) nogen keepus(langsos) 
				gen lep=.
				replace lep=0 if eng==1 | lanx==2
				replace lep=1 if inlist(eng,2,3,4)
				assert lep<. if agep>=5 
				keep state year langsos lanp16 lep lanx lanp agep sex
				if `i'>1 append using sos/donor_lanp_`1'.dta
				save sos/donor_lanp_`1'.dta, replace
				local ++i
			}
		}
		** tbd: check if all langc39 and langc42 languages have >=1 record, if there's non-zero population in a langc39/langc42 control.
	*}
	*else nois di ". Donor language file found; skipping"
end
*donorLang 2019
...
prog def langFile
	local year=`1'
	local y=substr("`year'",3,2)
	** load PUMS (later, keep state serialno sporder and don't load repwts)
	use state county year sex agep lanx lanp eng pw* using 5ACS`y'_ORWA_co.dta, clear
	gen stfips=state
	gen stcofips=state+county
	drop state county
	** language categories
	** you either merge langoha here as well, and keep it moving forward -- or you merge it later, as a consolidation of langsos.
	ren lanp lanp16 // only works for PUMS samples from 2016+
	merge m:1 lanp16 using SOS/prc_langxwalk.dta, keep(1 3) nogen keepus(langsos langsoslbl)
	labmask langsos, values(langsoslbl)
	drop langsoslbl
	** lep
	gen byte lep=.
	replace lep=0 if eng==1 | lanx==2
	...
	** (1) make a list of lang*lep that are missing for each county; (2) hotdeck from donor obs without geo restriction
	preserve
	tempfile tmp2
	drop if inlist(langsos,85,86,87,89,90,91,92) // african languages (ganda,kinyarwandan,shona,fulah,wolof,akan) w/o donors in region, and with another lang39/lang42/lang12 record
	drop if inlist(langsos,94,95,96,98,99,100) // aian languages (aleut/apache/dakota/keres/cherokee/ojibwa) EXCEPT navajo (because it's on the lang39 list)
	fillin year stcofips langsos lep // ensure complete list of langsos for each county
	keep if _fillin==1 & langsos<. & lep<. 
	replace stfips=substr(stcofips,1,2) if stfips==""
	keep year stfips stcofips langsos lep lanx lanp16 agep sex
	** fillin such that every county is rectangular by agec3-langc5-lep (for subsequent controls)
	** TBD
	** previously, used two donor datasets (5acs16, 5acs21) to ensure coverage. now using 10 yrs' preceding (>=2016)
	append using sos/donor_lanp_`year'.dta, keep(langsos lanp16 lep lanx lanp agep sex) gen(donor) 
	drop if inlist(langsos,85,86,87,89,90,91,92) // african languages (ganda,kinyarwandan,shona,fulah,wolof,akan) w/o donors in region, and with another lang39/lang42/lang12 record
	drop if inlist(langsos,94,95,96,98,99,100) // aian languages (aleut/apache/dakota/keres/cherokee/ojibwa) EXCEPT navajo=97 (because it's on the lang39/lang42 list)
	set seed 13371701
	bys langsos lep: hotdeckvar lanp16 lanx agep sex, suffix("_m1")
	bys langsos: hotdeckvar lanp16 lanx agep sex, suffix("_m2")
	drop if donor==1 
	for var lanp16 lanx agep sex: replace X=X_m1 if X==. & X_m1<. \\ replace X=X_m2 if X==. & X_m2<.
	drop donor* *_m1 *_m2
	assert sex<. & agep<. & lanp<. & lanx<. & langsos<. & lep<.
	keep if sex<. & agep<. & lanp<. & lanx<. & langsos<. & lep<.
	gen pwgtp=1e-3 
	save `tmp2'
	restore
	append using `tmp2'
	** (3) merge multnomah, washington control totals by langc42/lep; (4) impute traits for lang/lep combinations missing in pums but present in the synthetic data
	merge m:1 stcofips langsos lep using SOS/sos_lang42_topdown_5acs`y'.dta, keepus(n) // county synthetic langsos (from lang42)
	drop if _merge==2 & n==0 // drop new languages if zero speakers
	set seed 13371701
	bys langsos lep: hotdeckvar lanp16 lanx agep sex if langsos!=., suffix("_m1")
	bys langsos: hotdeckvar lanp16 lanx agep sex if langsos!=., suffix("_m2")
	...
	drop *_m1 *_m2 _merge
	assert sex<. & agep<. 
	assert lanp<. & lanx<. & langsos<. & lep<. if eng<.
	ren n lcfnlco_n // control 
	** summarized language categories
	merge m:m langsos using SOS/prc_langxwalk.dta, keep(1 3) nogen keepus(langsoslbl langc39 langc42 langc12 langc5)
	...
	** merge control totals by 2015 langc39 
	...
	merge m:1 stcofips langc39 lep using temp/control_langc39_tmp.dta, assert(1 3) keepus(lc39_n) // old county lang39 ~ if fails, you may have dropped a langc39 category above (drop if inlist...).
	ren lc39_n lc39co_n
	...
	** merge OR state totals by langc42/lep
	...
	merge m:1 stfips year langc42 lep using temp/control_langc42st_tmp.dta, assert(1 3) keepus(lc42st_n) // state total by lang42 ~ if fails, you may have dropped a langc42 category above (drop if inlist...).
	...
	*survwgt poststratify pwgtp, by(stcofips agec11 sex) totvar(as_n) gen(pwt0) 
	...
	gen byte touse=!inlist(stcofips,"41029","41019","41039","41017","41047","41051","41005","41067","53011") // exclude single-county PUMAs
	set seed 13371701
	...
	*tbd; will require more fillin and pre-seed by donor obs
	...
	set seed 13371701
	survwgt poststratify pwgtp if touse==1 & agep>=5, by(stcofips langsos lep) totvar(lcfnlco_n) gen(pwt2) // control to 2016+ detailed languages (for wash/mult counties)
	...
	*survwgt rake pwt1 if stfips=="53" & agep>=5, by(id2) totvars(lc12_n) gen(pwt2) maxrep(255)
	...
	tab stcofips if langsos==11 & stfips=="41" [iw=pwt3] // check that totals agree ~ OR 355,412 spanish in 5ACS22 `ok'
	destring stcofips, replace
	keep stcofips agep langsos langc42 lanx lep pwt3 pwgt* // keep only critical variables for the "lang" extract.
	compress
	// merge in langoha
	** this works ONLY if langoha is a subset of langsos.
	preserve
	tempfile lx
	use sos/prc_langxwalk.dta, clear
	contract langsos langsoslbl langoha langohalbl langohastr
	by langsos: gen listme=_n
	assert listme==1 // no duplicates, 1:1 match
	save `lx'
	restore
	merge m:1 langsos using `lx', keepus(langoha langohalbl langohastr) keep(1 3) 
	tab _merge lanx if agep>=5, mis // confirmed _m=1 for ENG, _m=3 for all other
	drop _merge
	labmask langoha, values(langohastr)
	// define weights, export
	qui for num 1/80: replace pwgtpX=pwgtpX*(pwt3/pwgtp) // rescale repwt
	svyset [pw=pwt3], sdr(pwgtp1-pwgtp80) vce(sdr) mse
	save 5ACS`y'_ORWA_lang.dta, replace
end
*langFile 2019
...
prog def laLabel
	...
	cap label var langsos "Detailed language (SOS)"
	cap label var langohalbl "Detailed language (Label)"
	cap label var langoha "Detailed language (ISO 639-3 code)"
end

// tables by langoha (subset of langsos, for languages in any county's top10 total or by lep) 
cap prog drop tablang
prog def tablang
	local y=substr("`1'",3,2)
	use 5ACS`y'_ORWA_lang.dta, clear
	...
	** roll up language categories for use by oha
	assert langoha<. if agep>=5 & lanx==1
	replace langoha=0 if agep>=5 & lanx==2 
	lab def langoha 0 "eng", modify
	** init table storage
	mat master=J(1,14,.)
	...
	** replace data with stored matrices
	...
	drop if stcofips==.
	laLabel `1'
	lab def AGEC3 -1 "Total" 5 "5-18" 19 "19-64" 65 "65+", replace
	lab values agecat AGEC3
	label var agecat "Broad Age Group (-1=Total; 5-18; 19-64; 65+)"
	recast byte agecat
	ren agecat agecat3
...
// add SOS variable langsos, collapse by table
...
prog define sosTable
	...
	use 5ACS`y'_ORWA_lang.dta, clear
	keep if agep>=5
	replace langsos=0 if agep>=5 & lanx==2 // English-only
	keep pwt3 langsos lep stcofips
	label define langsos 0 "English", add
	collapse (sum) pwt3, by(stcofips langsos lep)
	reshape wide pwt3, i(stcofips langsos) j(lep)
	...
	** tbd
	...
	collapse (sum) total* lep*, by(langsos)
	gen stcofips=41
	append using SOS/lang_table_`year'.dta 
	for var total* lep*: assert X<.
	order stcofips langsos total* lep*
	for var "total_`year'" "lep_`year'": format X %7.0f
	// drop English or Eng Creole
	drop if langsos==0 | langsos==1
	// check no 0 spanish totals (affected Gilliam 5ACS21)
	assert total_`year'>0 if langsos==11
	// change labels for Persian, Pashto, Chinese, Lao, Burmese, Khmer, Tagalog
	** deprecated; updated already, above.
	// imputation of AIAN languages to associated tribal areas in OR
	*replace langsos=203 if langsos==101 & inlist(stcofips,41059,41031,41065) // "UMATILLA","WASCO","JEFFERSON"
	*lab def langsos 203 "Sahaptin", add modify
	// drop groups
	drop if inlist(langsos,36,38,63,67,74,83,88,93,102,103) // drop unrankable and unidentifiable groups ~ except "Other AIAN"
...
prog def sosSplit
	...
	*/
	...
	censusapi, url("https://api.census.gov/data/`past'/acs/acs5?get=group(B05006)&for=county:*&in=state:41&key=$ckey")
	drop if inlist(county,51,67,3) // drop mul/cla/was from historical data
	*append using `tmp'
	save `tmp', replace
	censusapi, url("https://api.census.gov/data/`year'/acs/acs5?get=group(B05006)&for=county:*&in=state:41&key=$ckey")
	append using `tmp'
	save `tmp', replace
	censusapi, url("https://api.census.gov/data/`year'/acs/acs5?get=group(B05006)&for=county:011&in=state:53&key=$ckey")
	append using `tmp'
	save `tmp', replace
	censusapi, url("https://api.census.gov/data/`year'/acs/acs5?get=group(B05006)&for=state:41&key=$ckey")
	append using `tmp'
	rename b05006_*e e*
	...
	*replace Farsi=`r(mean)' if Farsi==. // use state mean for zero pop areas
	...
	use stcofips langsos pw* if inlist(langsos,25,48) using 5ACS`y'_ORWA_lang.dta, clear
	tostring stcofips, replace
	gen byte one=1
	svy sdr: total one if langsos==25, over(stcofips) 
	svy sdr: total one if langsos==25 & substr(strofreal(stcofips),1,2)=="41"
	svy sdr: total one if langsos==48, over(stcofips) 
	svy sdr: total one if langsos==48 & substr(strofreal(stcofips),1,2)=="41"
end
*sosSplit 2019
